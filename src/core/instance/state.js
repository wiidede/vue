...
function initProps (vm: Component, propsOptions: Object) {
  ...
  // 缓存prop键，以便将来的prop更新可以使用 Array 而不是动态对象键枚举进行迭代。
  ...
  for (const key in propsOptions) {
    ...
    // instantiation here.
    // 静态props已经在 Vue.extend() 期间代理到组件的原型上。我们只需要在这里代理实例化时定义的 props。
    if (!(key in vm)) {
      // 代理 key 到 vm 对象上
      proxy(vm, `_props`, key)
    }
...
function createComputedGetter (key) {
  ...
  return function computedGetter () {
    ...
    if (watcher) {
      ...
      // 因为上一次执行 watcher.evaluate 时把 watcher.dirty 置为了 false，
...
function initMethods (vm: Component, methods: Object) {
  ...
  for (const key in methods) {
		// 验证 1. vm[key] 是否为函数 2. 验证key是否和props中的key冲突 3. 验证key是否与vm上的全局方法冲突
    if (process.env.NODE_ENV !== 'production') {
      ...
      if ((key in vm) && isReserved(key)) {
		...
		// ToDo 了解为什么函数用的是bind()，而prop用的proxy()
